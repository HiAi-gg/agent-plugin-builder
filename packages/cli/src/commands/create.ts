...
export const createCommand = new Command('create')
  ...
  .option('--skill-description <description>', 'Description for the last --skill')
  ...
  .option('--mcp-args <args...>', 'MCP server args (repeatable)', collectArgs, [])
  .option('--mcp-env <KEY=VALUE>', 'MCP server env vars (repeatable)', collectEnv, {})
  ...
  .action(async (options: any) => {
    ...
    if (!options.name) {
      throw new Error('--name is required (or use --config with a plugin.yml file)');
    }
