...
export const migrateCommand = new Command('migrate')
  ...
  .action(async (sourceDir: string, options: any) => {
    ...
    if (options.from) {
      ...
      console.log(`Detected format: ${detection.format} (${detection.confidence} confidence)`);
    }

    // Migrate
    try {
      const plugin = await migrateSource(sourcePath, options.from as SourceFormat);
      ...
      if (plugin.sourceArtifacts.length > 0) {
        console.log('\nMigration report:');
        const portable = plugin.sourceArtifacts.filter((a) => a.classification === 'PORTABLE');
        ...
        if (clientSpecific.length > 0) {
          console.log(`  ⚠ Client-specific: ${clientSpecific.length} item(s) (not migrated)`);
        }
        if (unsupported.length > 0) {
          console.log(`  ✗ Unsupported: ${unsupported.length} item(s) (not migrated)`);
        }
      }
    } catch (error) {
