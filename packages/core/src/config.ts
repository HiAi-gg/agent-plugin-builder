...
const configSkillSchema = z.object({
  name: z.string().max(64).regex(/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/),
...
export function configToPortablePlugin(config: PluginConfig, configDir: string): PortablePlugin {
  ...
  for (const [name, server] of mcpEntries) {
    if (server.type === 'stdio') {
      if (!server.command) throw new Error(`MCP server "${name}" is stdio but missing command`);
      mcpServers.push({
        type: 'stdio',
      ...
      if (!server.url) throw new Error(`MCP server "${name}" is ${server.type} but missing url`);
      mcpServers.push({
        type: server.type,
        url: server.url,
  ...
  // Parse extensions
  const extensions: PortableExtension[] = Object.entries(config.extensions || {}).map(
